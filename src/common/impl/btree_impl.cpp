...
BTIterator::BTIterator(BTNode* root) : 
m_root(root), 
m_leaf(nullptr), 
m_index(0) 
{
...
btree_create_internal() 
{
  ...
  memset(&node->m_internal.m_keys[0],0,sizeof(uint32_t)*(BTREE_MAX_ARITY-1));
...
BTNode* btree_create_leaf() 
{
  ...
  memset(&node->m_leaf.m_keys[0],0,sizeof(uint32_t)*BTREE_MIN_ARITY);
...
btree_destroy_node(BTNode* node) 
{
  ...
  {
    for (uint32_t i = 0; i < BTREE_MAX_ARITY; ++i) 
    {
                     ...
                     uint32_t* sibling_key) 
{
  ...
  uint32_t start = (BTREE_MAX_ARITY+2-1)/2;
  *sibling_key = node->m_internal.m_keys[start-1];
  node->m_internal.m_keys[start-1] = 0;
  for(uint32_t i = start; i < BTREE_MAX_ARITY; ++i) 
  {
  ...
  for(uint32_t i = start; i < BTREE_MAX_ARITY-1; ++i ) 
  {
                 ...
                 uint32_t* sibling_key) 
{
  ...
  uint32_t start = (BTREE_MIN_ARITY+2-1)/2;
  *sibling_key = node->m_leaf.m_keys[start];
  for(uint32_t i = start; i < BTREE_MIN_ARITY; ++i ) 
  {
...
{
  assert(idx > 0);
  for(uint32_t i = BTREE_MAX_ARITY-1; i > idx; --i) 
  {
    node->m_internal.m_children[i] = node->m_internal.m_children[i-1];
  }

  for(uint32_t i = BTREE_MAX_ARITY-2; i > idx-1; --i) 
  {
...
btree_shift_insert_leaf(BTNode* node, 
                        ...
                        uint32_t key ) 
{
  for(uint32_t i = BTREE_MIN_ARITY-1; i > idx; --i) 
  {
...
void btree_insert(BTNode* node, uint32_t key, void* element) 
{
  if(node->m_type == BTNodeType::E_INTERNAL) 
  {
...
void btree_insert_root(BTNode** node, uint32_t key, void* element) 
{
  BTNode* root = *node;
...
btree_remove_shift_internal(BTNode* node, uint32_t idx) {
  ...
  for (uint32_t i = idx; i < BTREE_MAX_ARITY-1; ++i) 
  { 
    node->m_internal.m_children[i] = node->m_internal.m_children[i+1];
  }
  ...
  for (uint32_t i = start; i < BTREE_MAX_ARITY-2; ++i) 
  {
    node->m_internal.m_keys[i] = node->m_internal.m_keys[i+1];
  }
...
{
  for (uint32_t i = idx; i < BTREE_MIN_ARITY-1; ++i) 
  { 
    node->m_leaf.m_leafs[i] = node->m_leaf.m_leafs[i+1];
    node->m_leaf.m_keys[i] = node->m_leaf.m_keys[i+1];
  }
             ...
             uint32_t* new_min) 
{
  *min_changed = false;
  if(node->m_type == BTNodeType::E_INTERNAL) 
  {
    ...
    if(child_idx < BTREE_MAX_ARITY - 1 && 
