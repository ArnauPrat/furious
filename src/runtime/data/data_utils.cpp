...
copy_component_ptr(uint32_t chunk_size, 
                   ...
                   uint32_t num_tables)
{
  uint32_t other_block_id = (target / TABLE_BLOCK_SIZE);
  ...
  BlockCluster* cluster = (BlockCluster*)hashtable_get(hash_table, other_block_id);
  if(cluster != nullptr)
  {
    if(bitmap_is_set(&cluster->m_enabled, target % TABLE_BLOCK_SIZE))
    {
                         ...
                         BitTable* blacklist)
{
  ...
  FURIOUS_ASSERT(!bitmap_is_set(&block_cluster->m_global, 0) && "BlockCluster cannot be global");
