...
/*
void
fdb_txbtree_iter_init(fdb_txbtree_iter_t* iter, const fdb_txbtree_t* btree)
{
  iter->p_root = btree;
  iter->m_index = 0;
...
fdb_btree_iter_has_next(fdb_btree_iter_t* iter)
{
  ...
  fdb_btree_node_t* sibling = iter->p_leaf->m_leaf.m_next;
  if(sibling == NULL) 
  {
    return false;
  }
...
fdb_btree_iter_next(fdb_btree_iter_t* iter)
{
  ...
  {
    fdb_btree_entry_t entry = {0xffffffff,NULL};
    return entry;
  }
  ...
  fdb_btree_entry_t entry = {id, value};
  return entry;
}
*/
...
{
  fdb_btree_node_t* node = (fdb_btree_node_t*)fdb_pool_alloc_alloc(&btree->m_node_allocator, 
                                                                   FDB_BTREE_NODE_ALIGNMENT, 
                                                                   sizeof(fdb_btree_node_t), 
                                                                   -1); 
  memset(node,0,sizeof(fdb_btree_node_t));
               ...
               fdb_mem_allocator_t* allocator)
{
  ...
  fdb_pool_alloc_init(&btree->m_node_allocator, 
                      FDB_BTREE_ALIGNMENT, 
                      sizeof(fdb_btree_node_t),
                      FDB_BTREE_PAGE_SIZE,
                      mallocator);
...
fdb_btree_release(fdb_btree_t* root)
{
  fdb_btree_destroy_node(root, 
                         root->p_root);
  fdb_pool_alloc_release(&root->m_node_allocator);
  root->p_root = NULL;
}
                       ...
                       fdb_btree_node_t* node) 
{
  ...
  fdb_pool_alloc_free(&btree->m_node_allocator, 
                      node);
}

uint32_t
__fdb_btree_next_internal(const fdb_btree_node_t* node,
                          uint32_t key)
{
...
uint32_t 
__fdb_btree_next_leaf(const fdb_btree_node_t* node, 
                      uint32_t key) 
{
                   ...
                   uint32_t ekey) 
{
  if(node->m_type == E_INTERNAL) 
  {
    ...
    {
      fdb_btree_node_t* child = node->m_internal.m_children[child_idx];
      if(child != NULL ) 
      { 
...
{
  FDB_ASSERT(node->m_type == E_INTERNAL && "Node must be an internal node");
  FDB_RESTRICT(fdb_btree_node_t*) sibling = fdb_btree_create_internal(btree);
...
{
  FDB_ASSERT(node->m_type == E_LEAF && "Node must be a leaf node");
  FDB_RESTRICT(fdb_btree_node_t*) sibling = fdb_btree_create_leaf(btree);
...
void 
fdb_btree_shift_insert_internal(FDB_RESTRICT(fdb_btree_node_t*) node, 
                                uint32_t idx, 
                                FDB_RESTRICT(fdb_btree_node_t*) child, 
                                uint32_t key ) 
{
  FDB_ASSERT(node->m_type == E_INTERNAL && "Node must be an internal node");
...
void 
fdb_btree_shift_insert_leaf(fdb_btree_node_t* node, 
                            uint32_t idx, 
                            uint32_t key ) 
{
  FDB_ASSERT(node->m_type == E_LEAF && "Node must be a leaf node");
...
fdb_btree_node_t* 
fdb_btree_split_child_full(fdb_btree_t* btree, 
                           FDB_RESTRICT(fdb_btree_node_t*) node, 
                           uint32_t child_idx, 
                           uint32_t key)
{
  FDB_RESTRICT(fdb_btree_node_t*) child = node->m_internal.m_children[child_idx];
  if(child->m_type == E_INTERNAL && child->m_internal.m_nchildren == FDB_BTREE_INTERNAL_MAX_ARITY) 
  {
    uint32_t sibling_key;
    FDB_RESTRICT(fdb_btree_node_t*) sibling = fdb_btree_split_internal(btree, 
                                                                       child, 
                                                                       &sibling_key);
  ...
  else if( child->m_type == E_LEAF && child->m_leaf.m_nleafs == FDB_BTREE_LEAF_MAX_ARITY )
  {
    ...
    {
      FDB_RESTRICT(fdb_btree_node_t*) sibling = fdb_btree_split_leaf(btree, 
                                                                     child, 
                                                                     &sibling_key);
      fdb_btree_shift_insert_internal(node, child_idx+1, sibling, sibling_key );
...
fdb_btree_insert_t 
fdb_btree_insert_node(fdb_btree_t* btree, 
                      FDB_RESTRICT(fdb_btree_node_t*) node, 
                      uint32_t key, 
                      void* ptr) 
{
  ...
  { 
       ...
       node->m_leaf.m_leafs[pos] == NULL)
    {
      ...
      btree->m_size++;
      fdb_btree_insert_t insert = {.m_inserted = true, 
                                   .p_place = &node->m_leaf.m_leafs[pos]};
      return insert;
    }
      fdb_btree_insert_t insert = {.m_inserted = false, 
                                   .p_place = &node->m_leaf.m_leafs[pos]};
    return insert;
  }
...
fdb_btree_insert_t 
fdb_btree_insert(fdb_btree_t* btree, 
                 uint32_t key, 
                 void* ptr) 
{
  fdb_btree_node_t* root = btree->p_root;
  ...
  if(root->m_internal.m_nchildren == FDB_BTREE_INTERNAL_MAX_ARITY) 
  {
    ...
    FDB_RESTRICT(fdb_btree_node_t*) sibling = fdb_btree_split_internal(btree, 
                                                                       root, 
                                                                       &sibling_key);
    FDB_RESTRICT(fdb_btree_node_t*) new_root = fdb_btree_create_internal(btree);
  ...
  FDB_RESTRICT(fdb_btree_node_t*) child = root->m_internal.m_children[child_idx];
  if(child == NULL) 
  {
...
fdb_btree_remove_shift_internal(fdb_btree_t* btree, 
                                ...
                                uint32_t idx) {
  FDB_ASSERT(node->m_type == E_INTERNAL && "Node must be an internal node");
  FDB_RESTRICT(fdb_btree_node_t*) old = node->m_internal.m_children[idx];
...
void* 
fdb_btree_remove_shift_leaf(fdb_btree_node_t* node, uint32_t idx) 
{
  FDB_ASSERT(node->m_type == E_LEAF && "Node must be a leaf node");
                         ...
                         uint32_t idx2) 
{
  FDB_ASSERT(node->m_type == E_INTERNAL && "Node must be an internal node");
  assert(idx2 == idx1+1);
  FDB_RESTRICT(fdb_btree_node_t*) child1 = node->m_internal.m_children[idx1];
  FDB_RESTRICT(fdb_btree_node_t*) child2 = node->m_internal.m_children[idx2];
                      ...
                      uint32_t* new_min) 
{
  *min_changed = false;
  if(node->m_type == E_INTERNAL) 
  {
    ...
    {
      FDB_RESTRICT(fdb_btree_node_t*) child1 = node->m_internal.m_children[child_idx];
      FDB_RESTRICT(fdb_btree_node_t*) child2 = node->m_internal.m_children[child_idx+1];
...
void* 
fdb_btree_remove(fdb_btree_t* btree, 
                 uint32_t key) 
{
  bool min_changed;
  uint32_t new_min;
