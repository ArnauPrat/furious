...
TEST(BTreeTest, btree_create) 
{
  ...
  for (uint32_t i = 0; i < BTREE_MAX_ARITY-1; ++i) 
  {
  ...
  for (uint32_t i = 0; i < BTREE_MIN_ARITY; ++i) 
  {
...
TEST(BTreeTest, btree_next_internal) 
{
  ...
  for (uint32_t i = 1; i < BTREE_MAX_ARITY; ++i) 
  {
...
TEST(BTreeTest, btree_split_internal) 
{
  ...
  uint32_t max_key = 0;
  for (uint32_t i = 1; i < BTREE_MAX_ARITY -1 && node->m_internal.m_children[i+1] != nullptr; ++i) 
  {
    ASSERT_TRUE(node->m_internal.m_keys[i] > node->m_internal.m_keys[i-1]);
    max_key = node->m_internal.m_keys[i];
  }

  for (uint32_t i = 1; i < BTREE_MAX_ARITY -1 && sibling->m_internal.m_children[i+1] != nullptr; ++i) 
  {
...
TEST(BTreeTest, btree_split_leaf) 
{
  ...
  uint32_t max_key = 0;
  for (uint32_t i = 1; i < BTREE_MIN_ARITY && node->m_leaf.m_leafs[i] != nullptr; ++i) 
  {
    ASSERT_TRUE(node->m_leaf.m_keys[i] > node->m_leaf.m_keys[i-1]);
    max_key = node->m_leaf.m_keys[i];
  }

  for (uint32_t i = 1; i < BTREE_MIN_ARITY && sibling->m_leaf.m_leafs[i] != nullptr; ++i) 
  {
...
TEST(BTreeTest, btree_get) 
{
  ...
  for (uint32_t i = 0; i < BTREE_MAX_ARITY; ++i) 
  {
    ...
    for (uint32_t j = 0; j < BTREE_MIN_ARITY; ++j) 
    {
      uint32_t key = i*BTREE_MIN_ARITY+j;
...
TEST(BTreeTest, btree_remove_shift_internal) 
{
  ...
  for (uint32_t i = 0; i < BTREE_MAX_ARITY-2; ++i) 
  {
...
TEST(BTreeTest, btree_remove_shift_leaf) 
{
  ...
  for (uint32_t i = 0; i < BTREE_MIN_ARITY-1; ++i) 
  {
...
TEST(BTreeTest, BTree) 
{
  ...
  for (uint32_t i = 0; i <= BTREE_MAX_KEY; ++i) 
  {
    ...
    ASSERT_EQ(value, values[i]);
    ASSERT_EQ(value->m_val, static_cast<uint32_t>(i));
    delete value;
  }
